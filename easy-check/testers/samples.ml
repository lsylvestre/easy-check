...
let tree ?(depth=(int 2 5)) leaf node () = 
  ...
  let rec aux = function
  | 0 -> leaf ()
  | n -> node (fun () -> aux (n-1)) in             (* if Random.int 10 = 0 then leaf () else *)
  aux d

let tree_p ?(depth=(int 2 5)) leaf node v () = 
  let d = depth () in
  if d < 0 then invalid_arg "tree";
  let rec aux v = function
  | 0 -> leaf v
  | n -> node (fun v -> aux v (n-1)) v  in
  aux v d

(*

let tree_p2 ?(depth=(int 2 5)) leaf node () = 
  let d = depth () in
  if d < 0 then invalid_arg "tree";
  let rec aux = function
  | 0 -> leaf () 
  | n -> node (fun ?(next=pred) () -> aux (min (next n) (n-1)) in
  aux d
*)
