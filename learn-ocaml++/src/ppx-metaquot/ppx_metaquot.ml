...
(* ------ <edited for learn-ocaml> ------ *)
          | Pexp_extension({txt="ty";loc=l}, e) ->
              ...
              Exp.constraint_
                ...
                (Typ.constr ty_id [ty])
(* ------ </edited for learn-ocaml> ------ *)
(* ------ <edited for learn-ocaml (p6) > ------ *)
           | Pexp_extension({txt="code";loc=l}, e) ->
             let v = get_exp l e in
             let show = 
               let x = (exp_lifter !loc this) # lift_Parsetree_expression v in
               Exp.apply (Exp.ident {txt = Longident.(Ldot (Lident "Expr", "print")); 
                                     loc=l }) [(Nolabel, x)] 
             in
             let pos = Lexing.dummy_pos in
             let let_open m v = 
               let open Parsetree in 
                { pexp_desc = Pexp_open (Asttypes.Fresh, {Asttypes.txt = Longident.Lident m;
                  loc       = { Location.loc_start = pos; loc_end = pos; loc_ghost = true}}, v);
                  pexp_loc  = { Location.loc_start = pos; loc_end = pos; loc_ghost = true}; pexp_attributes = [] } 
             in
             tuple [ let_open "Code" v;
                     let_open "Solution" v;
                     show]
(* ------ </edited for learn-ocaml (p6) > ------ *)
           | _ ->
