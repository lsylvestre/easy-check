  ...
  | { Parsetree.ptyp_desc = Parsetree.Ptyp_package (n, rews); _ }, 'A'.. 'Z' ->
      begin match Env.lookup_module ~load:false lid !Toploop.toplevel_env with
        ...
        | path ->
            ...
            (* <> more readable message in case of signature mismatch (Quick-and-dirty) *)
            try if Toploop.execute_phrase false ppf phrase 
                then let fake_path, _ = Env.lookup_value (Longident.Lident "%fake%") !Toploop.toplevel_env in
                     Present (Obj.obj @@ Toploop.eval_path !Toploop.toplevel_env fake_path)
                else assert false 
            with _ -> (let open Types in 
                       let open Asttypes in
                       Printtyp.modtype ppf (Mtype.scrape !Toploop.toplevel_env (Env.find_module path !Toploop.toplevel_env).md_type);
                       Format.fprintf ppf "@." ; 
                       let your_implementation = Buffer.contents buf in
                       Buffer.clear buf;
                       let (_,m) = Env.lookup_modtype n.txt !Toploop.toplevel_env in
                       let m' = match m.mtd_type with None -> assert false | Some m -> m in
                       Printtyp.modtype ppf (Mtype.scrape !Toploop.toplevel_env m');        
                       Format.fprintf ppf "@." ;
                       let signature_expected = Buffer.contents buf in
            failwith ("Votre implémentation (dont la signature est :\n\n" ^
                      your_implementation ^ "\n) n'est pas compatible avec la signature attendue :\n\n" ^ signature_expected))
            (* </> *)
    end
  | _ ->
      ...
      (* <>
         see https://github.com/ocaml/ocaml/blob/trunk/typing/ctype.ml#L24 
         -> before calling Ctype.moregeneral,
            one must first make sure that the type levels are correct, 
            using the function Ctype.correct_levels. *)
      let val_type = Ctype.correct_levels val_type
      and exp_type = Ctype.correct_levels exp_type in 
      (* </> *)
      if (Ctype.opened_object exp_type 
          && (try Ctype.subtype !Toploop.toplevel_env val_type exp_type (); true with Ctype.Subtype _ -> false))
         || Ctype.moregeneral !Toploop.toplevel_env true val_type exp_type then
...
let sample_value ty =
  ...
  | true ->
    ...
    (* <> see https://github.com/ocaml/ocaml/blob/trunk/typing/ctype.ml#L24 *)
    let val_type = Ctype.correct_levels val_type
    and gty = Ctype.correct_levels gty in
    (* </> *)
